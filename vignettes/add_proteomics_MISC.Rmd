---
title: "MultiNicheNet analysis: MIS-C: add serum proteomics data modality"
author: "Robin Browaeys"
package: "`r BiocStyle::pkg_ver('multinichenetr')`"
output: 
  BiocStyle::html_document
vignette: >
  %\VignetteIndexEntry{MultiNicheNet analysis: MIS-C: add serum proteomics data modality}
...
Before incorporating the serum proteomics, we will have a look at the output of this regular MultiNicheNet analysis.
...
olink_df_ligand_scaled = olink_df_ligand %>% 
  ...
  dplyr::mutate(
    scaled_lfc_ligand_OLINK = rank(logFC, ties.method = "average", na.last = FALSE)/max(rank(logFC, ties.method = "average", na.last = FALSE)), 
    scaled_p_val_ligand_adapted_OLINK = rank(pval_adapted, ties.method = "average", na.last = FALSE)/max(rank(pval_adapted, ties.method = "average", na.last = FALSE))) 
...
olink_df_ligand_scaled = olink_df_ligand_scaled %>% 
  ...
  dplyr::arrange(-scaled_p_val_ligand_adapted_OLINK)
...
regular_criteria_tbl = tibble(
  ...
  ) # do not change this  - logFC and pval of ligand and receptor get 0.5 as regularization such that DE of the ligandcounts as one score, which is the sum of the scaled_score of the logFC and the p-value-based score. This also applies to the receptor.

new_criteria_tbl = tibble(
  criterion = c("scaled_lfc_ligand_OLINK","scaled_p_val_ligand_adapted_OLINK"), weight = c(1,1), regularization_factor = c(0.5, 0.5)) # 0.5 as regularization such that DE of the ligand at the protein level counts as one score, which is the sum of the scaled_score of the logFC and the p-value-based score. 
```
...
```{r}
group_prioritization_tbl_withoutOLINK %>% select(group, id, prioritization_score) %>%
  inner_join(
...
group_prioritization_tbl_withoutOLINK %>% 
  select(group, id, prioritization_score) %>% 
  inner_join(
    prioritization_tables$group_prioritization_tbl %>% 
      select(group, id, prioritization_score) %>% 
  ...
  mutate(difference = new_score - prioritization_score) %>% 
  arrange(difference) %>% 
  filter(prioritization_score > 0.80)
```
