...
```{r}
calculate_de_findmarkers = function(sce, celltype_id, group_id, genes_filtered){
  celltypes = SummarizedExperiment::colData(sce)[,celltype_id] %>% unique()

  DE_table = celltypes %>% lapply(function(celltype_oi, sce){
...
genes_filtered = DE_info$celltype_de$de_output_tidy %>% dplyr::pull(gene) %>% unique()
celltype_de_findmarkers = calculate_de_findmarkers(sce, celltype_id, group_id, genes_filtered)
```
...
!!!!!!!!!!!!!!!!!!!!! NEW NEW NEW !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
https://bioconductor.org/books/release/OSCA/marker-detection.html
Try here to implemenet naive DE method that we can use as check to compare MultiNicheNet against!

# tibble with following columns: gene, cluster_id, logFC, (logCPM), F, p_val, p_adj.loc, p_adj, contrast
# naive test: can also include factors to block on... (check the design and block arguments -- really need to have a good comparison...)
# is this the golden grail instead of muscat: wilcox per block? no because this does not allow to compare patient groups ...
...
Before, we had calculated the correlation in expression between ligand-receptor pairs and DE genes. Now we will filter out correlated ligand-receptor --> target links that both show high expression correlation (spearman or pearson > 0.75 in this example) and have some prior knowledge to support their link.

To filter we can use one of these two options: `lr_target_prior_cor_filtered` (recommended and based on scaled ligand-target prior knowledge: the target should be a top target of the ligand, and the ligand should be a top ligand for the target) or `lr_target_prior_cor_filtered_original` (based on the same ligand-target df returned by default in classic NicheNet: focuses on top targets of a ligand only, and not necessarily the reverse)

```{r}
group_oi = "High"
receiver_oi = "Malignant"
lr_target_prior_cor_filtered = multinichenet_output$lr_target_prior_cor %>% filter(scaled_prior_score > 0.50 & (pearson > 0.75 | spearman > 0.75))

lr_target_prior_cor_filtered_original = multinichenet_output$lr_target_prior_cor %>% dplyr::inner_join(multinichenet_output$ligand_activities_targets_DEgenes$ligand_activities %>% dplyr::inner_join(contrast_tbl) %>% dplyr::filter(group == group_oi) %>% dplyr::ungroup() %>% dplyr::distinct(ligand, target), by = c("ligand", "target")) %>% dplyr::filter(pearson > 0.75 | spearman > 0.75)
```
...
lr_target_prior_cor_heatmap = make_lr_target_prior_cor_heatmap(lr_target_prior_cor_filtered %>% filter(receiver == receiver_oi & id %in% prioritized_tbl_oi$id), add_grid = FALSE)
lr_target_prior_cor_heatmap
lr_target_prior_cor_heatmap = make_lr_target_prior_cor_heatmap(lr_target_prior_cor_filtered %>% filter(receiver == receiver_oi & id %in% prioritized_tbl_oi$id), add_grid = TRUE)
lr_target_prior_cor_heatmap
```
...
lr_target_correlation_plot = make_lr_target_correlation_plot(multinichenet_output$prioritization_tables, prioritized_tbl_oi, lr_target_prior_cor_filtered, multinichenet_output$grouping_tbl, multinichenet_output$celltype_info, receiver_oi,plot_legend = FALSE, show_cor = FALSE)
lr_target_correlation_plot$combined_plot
lr_target_correlation_plot = make_lr_target_correlation_plot(multinichenet_output$prioritization_tables, prioritized_tbl_oi, lr_target_prior_cor_filtered, multinichenet_output$grouping_tbl, multinichenet_output$celltype_info, receiver_oi,plot_legend = FALSE, show_cor = TRUE)
lr_target_correlation_plot$combined_plot
```
