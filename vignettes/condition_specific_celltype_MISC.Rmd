...
condition_specific_celltypes = abundance_df_summarized %>% filter(samples_present == 0) %>% pull(celltype_id) %>% unique()
...
## Step A-2 Visualization of the results of the cell-cell communication analysis
...
```{r}
prioritized_tbl_oi_all = get_top_n_lr_pairs(prioritization_tables_with_condition_specific_celltype_sender, 50, rank_per_group = FALSE)
```

```{r, fig.width=15, fig.height=12}
prioritized_tbl_oi = prioritization_tables_with_condition_specific_celltype_sender$group_prioritization_tbl %>%
  filter(id %in% prioritized_tbl_oi_all$id) %>%
  distinct(id, sender, receiver, ligand, receptor, group) %>% left_join(prioritized_tbl_oi_all)
prioritized_tbl_oi$prioritization_score[is.na(prioritized_tbl_oi$prioritization_score)] = 0 
```
...
plot_oi = make_sample_lr_prod_activity_plots(prioritization_tables_with_condition_specific_celltype_sender, prioritized_tbl_oi_S_50)
plot_oi
```
...
plot_oi = make_sample_lr_prod_activity_plots(prioritization_tables_with_condition_specific_celltype_sender, prioritized_tbl_oi_M_50)
plot_oi
```
...
plot_oi = make_sample_lr_prod_activity_plots(prioritization_tables_with_condition_specific_celltype_sender, prioritized_tbl_oi_A_50)
plot_oi
```
...
```
We see that some top-predicted interactions are coming from the condition-specific cell type. This means that these ligands are the most strongly expressed by that cell type, and that they have quite high ligand activity.

## Step B-2 Visualization of the results of the cell-cell communication analysis
...
```{r}
prioritized_tbl_oi_all = get_top_n_lr_pairs(prioritization_tables_with_condition_specific_celltype_receiver, 50, rank_per_group = FALSE)
```

```{r, fig.width=15, fig.height=12}
prioritized_tbl_oi = prioritization_tables_with_condition_specific_celltype_receiver$group_prioritization_tbl %>%
  filter(id %in% prioritized_tbl_oi_all$id) %>%
  distinct(id, sender, receiver, ligand, receptor, group) %>% left_join(prioritized_tbl_oi_all)
prioritized_tbl_oi$prioritization_score[is.na(prioritized_tbl_oi$prioritization_score)] = 0 
```
...
plot_oi = make_sample_lr_prod_activity_plots(prioritization_tables_with_condition_specific_celltype_receiver, prioritized_tbl_oi_S_50)
plot_oi
```
...
plot_oi = make_sample_lr_prod_activity_plots(prioritization_tables_with_condition_specific_celltype_receiver, prioritized_tbl_oi_M_50)
plot_oi
```
...
plot_oi = make_sample_lr_prod_activity_plots(prioritization_tables_with_condition_specific_celltype_receiver, prioritized_tbl_oi_A_50)
plot_oi
```
...
```{r}
prioritized_tbl_oi_all = get_top_n_lr_pairs(combined_prioritization_tables, 50, rank_per_group = FALSE)
```

```{r, fig.width=15, fig.height=12}
prioritized_tbl_oi = combined_prioritization_tables$group_prioritization_tbl %>%
  filter(id %in% prioritized_tbl_oi_all$id) %>%
  distinct(id, sender, receiver, ligand, receptor, group) %>% left_join(prioritized_tbl_oi_all)
prioritized_tbl_oi$prioritization_score[is.na(prioritized_tbl_oi$prioritization_score)] = 0 
```
