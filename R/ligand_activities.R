...
#' @importFrom parallel makeCluster parLapply clusterExport stopCluster clusterEvalQ detectCores
#'
#' @examples
#' \dontrun{
#' library(dplyr)
...
#' @title get_ligand_activities_targets_DEgenes_beta
#'
#' @description \code{get_ligand_activities_targets_DEgenes_beta}  BETA-version with new parallelization functionality -- Predict NicheNet ligand activities and ligand-target links for the receiver cell types. Uses `nichenetr::predict_ligand_activities()` and `nichenetr::get_weighted_ligand_target_links()` under the hood.
#' @usage get_ligand_activities_targets_DEgenes_beta(receiver_de, receivers_oi,  ligand_target_matrix, logFC_threshold = 0.50, p_val_threshold = 0.05, p_val_adj = FALSE, top_n_target = 250, verbose = FALSE, n.cores = 1)
#'
...
#' @export
#'
get_ligand_activities_targets_DEgenes_beta = function(receiver_de, receivers_oi,  ligand_target_matrix, logFC_threshold = 0.50, p_val_threshold = 0.05, p_val_adj = FALSE, top_n_target = 250, verbose = FALSE, n.cores = 1){
  ...
  if(n.cores_oi > 1){
    ...
    ligand_activities_targets_geneset_ALL = foreach(i = 1:length(receivers_oi), .packages = c("dplyr", "nichenetr", "muscat", "tidyr", "tibble", "purrr")) %dopar% {
      
      receiver_oi = receivers_oi[i]      
